...
export async function syncGame(
  ...
  try {
    return await invoke<SyncResultDto>('sync_game', { gameId, auth });
  } catch (error) {
...
export async function getGameSaveStats(
  gameId: string
): Promise<GameSaveStatsDto> {
  try {
...
export async function getVersionAnalysis(
  ...
  try {
    const raw = await invoke<string | null>('get_version_analysis', {
      versionId,
    });
    if (!raw) return null;
...
export async function saveVersionAnalysis(
  versionId: string,
  data: any
): Promise<void> {
  try {
...
export async function deleteVersionAnalyses(
  versionIds: string[]
): Promise<number> {
  try {
    return await invoke<number>('delete_version_analyses', { versionIds });
  } catch (error) {
    console.error('Failed to delete version analyses:', error);
    return 0;
  }
}
