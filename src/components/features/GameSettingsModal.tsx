...
export default function GameSettingsModal({ isOpen, onClose, game }: GameSettingsModalProps) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            toast.error('Save Failed', error instanceof Error ? error.message : 'Unknown error occurred')
        } finally {
            setIsSaving(false)
        }
    }
    ...
    return (
        <Modal isOpen={isOpen} onOpenChange={(open) => !open && onClose()}>
            <Modal.Backdrop variant="blur">
                <Modal.Container>
                    <Modal.Dialog className="bg-bg-card border border-white/10 shadow-2xl rounded-2xl max-w-lg">
                        <Modal.CloseTrigger />
                        <Modal.Header className="border-b border-white/5 p-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <Modal.Heading className="text-xl font-bold text-white tracking-tight">Game Settings</Modal.Heading>
                                    <p className="text-xs text-gray-500 mt-0.5">Configure sync options for this game</p>
                                </div>
                            </div>
                        </Modal.Header>
                        <Modal.Body className="p-6">
                            <div className="space-y-6">
                                ...
                                {/* Platform */}
                                <div className="space-y-1.5">
                                    <Label className="text-sm text-gray-400 font-medium ml-1">Platform</Label>
                                    <Select
                                        selectedKey={platform}
                                        onSelectionChange={(key) => setPlatform(key as GamePlatform)}
                                    >
                                        <Select.Trigger className="w-full h-12 bg-bg-elevated border border-white/10 rounded-xl px-4 text-white hover:border-primary-500/50 transition-colors">
                                            <Select.Value />
                                            <Select.Indicator />
                                        </Select.Trigger>
                                        <Select.Popover>
                                ...
                                <div className="p-4 rounded-xl bg-bg-elevated/30 border border-white/5 flex items-center justify-between">
                                    <div className="space-y-0.5">
                                        <p className="text-sm font-semibold text-white">Auto-Sync Enabled</p>
                                        <p className="text-[11px] text-gray-500 font-medium">Automatically backup saves when changes are detected</p>
                                    </div>
                                    <Switch
