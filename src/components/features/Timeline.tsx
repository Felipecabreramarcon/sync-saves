...
function TimelineItem({ activity }: { activity: SyncActivity }) {
    ...
    const actionMap: Record<string, { icon: any, label: string, color: string }> = {
        upload: { icon: ArrowUpCircle, label: "Pushed to cloud", color: "text-success" },
        download: { icon: ArrowDownCircle, label: "Pulled from cloud", color: "text-info" },
        conflict: { icon: GitMerge, label: "Merge conflict", color: "text-warning" },
        skip: { icon: GitPullRequest, label: "Skipped sync", color: "text-gray-400" },
    };
    ...
    const handleRestore = async () => {
        try {
            ...
            toast.success('Save Restored', `Restored save from ${format(new Date(activity.created_at), "MMM d, HH:mm")}`)
        } catch (error) {
    ...
    const handleDownload = async () => {
        try {
            ...
            // Download blob
            const { data, error } = await supabase.storage.from('saves').download(versionInfo.filePath)
            if (error || !data) throw error || new Error('Download failed')

            // Ask user where to save
            const defaultName = `${activity.game_name.replace(/[^a-z0-9]/gi, '_')}_${format(new Date(activity.created_at), "yyyyMMdd_HHmm")}.zip`
            ...
            await invoke('write_file', { path: savePath, content: Array.from(uint8Array) })
    ...
    const canRestore = activity.action === 'upload' && activity.save_version_id && activity.status === 'success'
                    ...
                    <div className="flex items-start gap-3 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2 flex-wrap">
                                <h4 className="font-bold text-white text-sm truncate max-w-[180px]">{activity.game_name}</h4>
                                <span className={cn("text-xs font-medium flex items-center gap-1 shrink-0", actionInfo.color)}>
                            ...
                            <div className="mt-1 flex items-center gap-2 text-xs text-gray-500 flex-wrap">
                                <span className="font-mono">{format(new Date(activity.created_at), "HH:mm")}</span>
                                ...
                                {activity.device_name && (
                                    <span className="text-gray-600">via {activity.device_name}</span>
                                )}
                            </div>
