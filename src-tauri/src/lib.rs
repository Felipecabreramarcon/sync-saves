    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            */
            ...
            {
               use tauri_plugin_deep_link::DeepLinkExt;
               // We can't easily grab specific schemes from config here in code without parsing config, 
               // so we hardcode "sync-saves" or just rely on the plugin to read it if it's smart (it's not always).
               // The plugin usually requires manual registration in setup for Windows/Linux.
